...
// Validate parameters
if ($volunteer_id <= 0 || empty($doc_type)) {
    http_response_code(400);
    die('Invalid parameters');
}

// Validate document type
$allowed_types = ['id_proof', 'certificate'];
...
try {
    ...
    // Fetch volunteer and document path
    $column_name = $doc_type . '_path';
    $query = "SELECT full_name, {$column_name} FROM volunteers WHERE id = ?";
    ...
    $file_path = $volunteer[$column_name];
    if (empty($file_path)) {
        http_response_code(404);
    ...
    $doc_label = ($doc_type == 'id_proof') ? 'ID_Proof' : 'Certificate';
    $download_filename = "Volunteer_{$volunteer_name}_{$doc_label}.{$file_extension}";
